...
def build(stock_id,name,dividends,prices):
 ...
 for year,g in grouped:
  px=p[p.year==year].sort_values("date") if not p.empty else pd.DataFrame(); close=px.iloc[-1].get("close",np.nan) if not px.empty else np.nan; cash=g.CashEarningsDistribution.map(num).sum(); reserve=g.CashStatutorySurplus.map(num).sum(); stock=g.StockEarningsDistribution.map(num).sum(); stock_reserve=g.StockStatutorySurplus.map(num).sum(); total_cash=cash+reserve; total_stock=stock+stock_reserve; total=total_cash+total_stock
  row={"stock_code":str(stock_id).zfill(4),"company_name":name,"股利_發放_期間":str(year),"股利_所屬_期間":str(int(g.belong_year.dropna().iloc[0])) if g.belong_year.notna().any() else "-","現金股利_盈餘":cash,"現金股利_公積":reserve,"現金股利_合計":total_cash,"股票股利_盈餘":stock,"股票股利_公積":stock_reserve,"股票股利_合計":total_stock,"股利_合計":total,"填息_花費_日數":"-","填權_花費_日數":"-","股價_年度":close,"file_type":"DividendDetail","source_file":f"FinMind_API_TaiwanStockDividend_{str(stock_id).zfill(4)}","download_success":True,"download_timestamp":stamp,"process_timestamp":stamp,"stage1_process_timestamp":stamp}
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--stock-id',default='2330');p.add_argument('--company-name',default='台積電');p.add_argument('--start-date',default='2018-01-01');p.add_argument('--end-date',default=(datetime.now()+timedelta(hours=8)).strftime('%Y-%m-%d'));p.add_argument('--output',default='financial/type1/raw_dividends_2330.csv');p.add_argument('--token',default=None);a=p.parse_args();rot=TokenRotator(a.token); d=fetch_data('TaiwanStockDividend',data_id=a.stock_id,start_date=a.start_date,end_date=a.end_date,token=rot); px=fetch_data('TaiwanStockPrice',data_id=a.stock_id,start_date=a.start_date,end_date=a.end_date,token=rot); out=build(a.stock_id,a.company_name,d.to_dict('records'),px.to_dict('records'));
