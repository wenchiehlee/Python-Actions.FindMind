...
def build(stock_id,name,price,inst,margin):
 ...
 for ws,g in p.groupby('_week',sort=True):
  ...
  mr=m[m.date.isin(g.date)].sort_values('date'); ml=mr.iloc[-1] if not mr.empty else pd.Series()
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--stock-id',default='2330');p.add_argument('--company-name',default='台積電');p.add_argument('--start-date',default='2021-01-01');p.add_argument('--end-date',default=(datetime.now()+timedelta(hours=8)).strftime('%Y-%m-%d'));p.add_argument('--output',default='financial/type11/raw_weekly_trading_data_2330.csv');p.add_argument('--token',default=None);a=p.parse_args();rot=TokenRotator(a.token); price=fetch_data('TaiwanStockPrice',data_id=a.stock_id,start_date=a.start_date,end_date=a.end_date,token=rot); inst=fetch_data('TaiwanStockInstitutionalInvestorsBuySellWide',data_id=a.stock_id,start_date=a.start_date,end_date=a.end_date,token=rot); margin=fetch_data('TaiwanStockMarginPurchaseShortSale',data_id=a.stock_id,start_date=a.start_date,end_date=a.end_date,token=rot); out=build(a.stock_id,a.company_name,price,inst,margin);
